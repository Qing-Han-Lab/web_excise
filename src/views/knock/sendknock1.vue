...
.shadowbox .shadowpic {
  ...
  background-image: url(./img/successknock.png);
  background-size: 100% 100%;
}
  ...
  <div class="allview">
    ...
    <div class="inputbox" v-if="pageitem.first==1">
      ...
      <div class="oneitem" v-if="pageitem.first==1&&pageitem.second==1">
        ...
        <div class="imgbox">
          <img id="orderpic1" :src="shareImg?shareImg : require('./img/m1.png')" alt />
    ...
    <div class="inputbox" v-if="pageitem.first==2">
      ...
      <div class="oneitem">
        ...
        <div class="imgbox">
          <img id="orderpic2" :src="shareImg?shareImg : require('./img/m1.png')" alt />
    ...
    <div class="submit" @click="submit">{{$route.query.isSave ? '保存爆料' : '发布爆料'}}</div>
    ...
    submit () {
      if (this.$route.query.isSave) {
        ...
        if (this.check()) {
          ...
          obj.sourceContentId = data.contentId
          if (data.contentId !== null) {
            obj.contentId = data.contentId
          }
          if (this.inputs1.salereason) {
            obj.productIntro = this.inputs1.salereason
          }
          if (this.inputs1.oldprice) {
            obj.currentPrice = this.inputs1.oldprice
          }
          if (this.inputs1.couponprice) {
            obj.discountPrice = this.inputs1.couponprice
          }
          if (this.inputs1.saleprice) {
            obj.realMoney = this.inputs1.saleprice
          }
          if (this.isNotice === true) {
            // 预告
            obj.isForecast = 1
            /*   if (this.inpust1.noticeprice) {
              obj.forecastPrice = this.inputs1.noticeprice
            } else {
              obj.forecastPrice = 0
            } */

            obj.forecastPrice = parseFloat(this.inputs1.noticeprice)
          ...
          // console.log(this.inputs1.actBeginTime)
          console.log(obj.isForecast, '预告')
          this.$http('POST', this.api.resetSingalKnock, {
            upJsonStr: JSON.stringify(obj)
          }).then(res => {
        ...
        if (this.check()) {
          ...
          if (this.pageitem.first === 1) {
            ...
            // 活动
            api = this.api.saveActiveItem
            let obj = {}
            obj.discountActName = this.inputs3.activename
            obj.concessionalDegree = this.inputs3.activepower
            if (data.platform) {
              obj.goodPlatform = data.platform
            }
            if (data.couponInfo) {
              ...
              if (data.couponInfo.couponUrl) {
                obj.couponUrl = data.couponInfo.couponUrl
              }
            }
            if (this.shareImg !== '') {
              obj.goodsCoverUrl = document.getElementById('orderpic2').src
            }
            if (data.itemPicUrl) {
              obj.goodsImgUrls = data.itemPicUrl
            }
            obj.recomendReason = this.inputs3.givereason
            obj.couponUrl = data.itemUrl
            this.$http('POST', api, {
              saveJsonStr: JSON.stringify(obj)
            }).then(res => {
              if (res.data.success === true) {
